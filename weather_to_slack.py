...
def outfit_suggestion(tmin, tmax, pop, rain):
    ...
    addtxt=f" (추가: {', '.join(addon)})" if addon else ""
    return f"상의: {top}, 하의: {bottom}{addtxt}"
...
def main():
    ...
    line1 = f"오늘의 기온은 (최저 {w['tmin']}도 / 최고 {w['tmax']}도)이며, 날씨는 {cond}입니다."
    line2 = f"오늘의 추천 옷차림 : {outfit_suggestion(w['tmin'], w['tmax'], w['pop'], w['rain'])}"
